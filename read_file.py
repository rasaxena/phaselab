...
plt.scatter(np_meas_dist,np_meas_dose)
plt.show()
...
print(len(fresh_x))
print(len(fresh_y))
sim_gamma_dose = interp1d(np_sim_dose_clipped[:len(np_sim_dose_clipped)/2],np_sim_dist_clipped[:len(np_sim_dist_clipped)/2],kind='cubic' )
meas_gamma_dose = interp1d(np_meas_dose[:len(np_meas_dose)/2],np_meas_dist[:len(np_meas_dist)/2],kind='cubic')
...
x_new_dose = np.linspace(np.min(np_meas_dose), np.max(np_meas_dose),1000, endpoint=True)
print(x_new_dose)
# print(x_new_dose)
sim_inter = sim(xnew)
...
print("interval")
print(sim_inter)
print ( [ix for ix in sim_inter if 0.2<ix < 0.95])

print ( [ix for ix in meas_inter if 0.2<ix < 0.95])

# plt.plot(xnew, sim_inter, '-', xnew, meas_inter, '--', xnew , gamma_dist , 'o')
# plt.legend(['data', 'linear', 'cubic'], loc='best')
# plt.show()
# fig,ax = plt.subplots()
# s = 121
